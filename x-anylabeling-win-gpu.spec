...
# Collect torch completely (GPU version)
...
# Collect onnxruntime-gpu
onnx_datas, onnx_binaries, onnx_hiddenimports = collect_all('onnxruntime')

# Get site-packages path for additional CUDA DLLs
site_packages = Path(sysconfig.get_path("purelib"))
onnxruntime_dlls = []
cuda_dll_path = site_packages / 'onnxruntime/capi/onnxruntime_providers_cuda.dll'
shared_dll_path = site_packages / 'onnxruntime/capi/onnxruntime_providers_shared.dll'
if cuda_dll_path.exists():
    onnxruntime_dlls.append((str(cuda_dll_path), 'onnxruntime/capi'))
if shared_dll_path.exists():
    onnxruntime_dlls.append((str(shared_dll_path), 'onnxruntime/capi'))

# Merge all
all_binaries = torch_binaries + ultralytics_binaries + onnx_binaries + onnxruntime_dlls
